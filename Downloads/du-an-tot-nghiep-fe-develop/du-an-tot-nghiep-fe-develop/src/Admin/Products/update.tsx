...
const UpdateProduct: React.FC = ({ show, handleCreate, getData , selected }: any) => {
  const [form] = Form.useForm()
  const onFinish = async (values: any) => {
  
    await instaceAxios.patch(`products/${selected._id}`, {
      ...values,
  ...
  return (
    <>
      <Modal title='Update Product' open={show} onCancel={() => handleCancel()} footer={[]}>
        <Form
         form={form}
        ...
        >
          ...
          <Form.Item label='Hình ảnh' name='thumbnail' rules={[{ required: true, message: 'Không được để trống!' }]}>
            <Upload {...props}>
